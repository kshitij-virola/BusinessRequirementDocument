...
import { cn, modeToFramework, deriveInputMode, getErrorMessage } from "@/lib/utils";
import { workspacesApi } from "@/lib/api/workspaces";
import { generationsApi } from "@/lib/api/generations";
import { invalidateWorkspaces, invalidateDashboard, invalidateMe } from "@/lib/api/hooks";
...
export const ChatPage = () => {
  ...
  const isGenerating = currentThread?.project?.status === "generating";
    ...
    onComplete: (data) => {
      ...
      if (state.pendingGenerationId) {
        generationsApi
          ...
          .then(() => Promise.all([invalidateDashboard(), invalidateMe()]))
          .catch(console.error)
  ...
  const handleSendMessage = async (
    ...
    try {
      ...
      // Existing thread: create the message and reuse the workspace created
      // when this thread was started (tracked in the chat store) — a follow-up
      // message must not spawn another workspace.
      ...
      const workspaceId = useChatStore.getState().activeWorkspaceId;
      if (!workspaceId) {
        console.error("No active workspace for this thread; skipping generation.");
      } else {
        generationsApi
          .generate({
            workspaceId,
          ...
          <div className="border-b border-border bg-background px-5 py-3.5 shrink-0">
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-full truncate">
                <Logo size="sm" href="/" className="mb-2 inline-block" />
                <div className="flex items-center gap-2 min-w-0">
                  <Link
                    href="/workspaces/chat"
              ...
              <div className="flex items-center gap-1 shrink-0">
                {/* <button type="button" className="flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs font-medium text-muted hover:bg-secondary hover:text-foreground transition-colors lg:hidden"><Code className="h-3.5 w-3.5" /><span>Code</span></button> */}
