...
// Read-only views onto the shared ai-theme-generator (Django) database
import legacyUserRoutes    from './routes/legacy/users'
import legacyProjectRoutes from './routes/legacy/projects'
import legacyThreadRoutes  from './routes/legacy/threads'
...
const limiter = rateLimit({
  windowMs: isDev ? 60_000 : env.rateLimit.windowMs,
  max:      isDev ? 500    : env.rateLimit.max,
...
const authLimiter = rateLimit({
  windowMs: isDev ? 60_000 : 15 * 60 * 1000,
  max:      isDev ? 50     : 20,
...
// Read-only views onto the shared ai-theme-generator database (kept from the
// earlier introspection phase, never touched by the routes above)
app.use('/api/legacy/users',    legacyUserRoutes)
app.use('/api/legacy/projects', legacyProjectRoutes)
app.use('/api/legacy/threads',  legacyThreadRoutes)
...
const bootstrap = async (): Promise<void> => {
  ...
  server.listen(env.port, () => {
    logger.info(`TROO AI Backend-PG API running on port ${env.port} (${env.nodeEnv})`)
  })
}
